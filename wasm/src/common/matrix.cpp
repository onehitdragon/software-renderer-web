...
M4x4 multi_matrix4x4(const M4x4 &matrix_a, const M4x4 &matrix_b){
    M4x4 result;
...
M4x1 multi_matrix4x4(const M4x4 &matrix_a, const M4x1 &matrix_b){
    M4x1 result;
    for(int i = 0; i < matrix_a.getM(); i++){
        for(int j = 0; j < matrix_b.getN(); j++){
            for(int k = 0; k < matrix_a.getN(); k++){
...
Vec3 M4x1_to_vec3(const M4x1 &m4x1){
    Vec3 result = {
        m4x1.value[0][0],
        m4x1.value[1][0],
        m4x1.value[2][0]
